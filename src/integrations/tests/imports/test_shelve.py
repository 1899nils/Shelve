...
class ImportShelve(TestCase):
    """Test importing media from Shelve CSV."""

    def setUp(self):
        """Create user for the tests."""
        self.credentials = {"username": "test", "password": "12345"}
        self.user = get_user_model().objects.create_user(**self.credentials)
        with Path(mock_path / "import_shelve.csv").open("rb") as file:
            self.import_results = shelve.importer(file, self.user, "new")
    ...
    def test_missing_metadata_handling(self):
        ...
        importer = shelve.ShelveImporter(None, self.user, "new")
...
class ImportShelvePartials(TestCase):
    """Test importing shelve media with no ID."""

    def setUp(self):
        """Create user for the tests."""
        self.credentials = {"username": "test", "password": "12345"}
        self.user = get_user_model().objects.create_user(**self.credentials)
        with Path(mock_path / "import_shelve_partials.csv").open("rb") as file:
            self.import_results = shelve.importer(file, self.user, "new")
...
class ImportShelveLists(TestCase):
    """Test importing shelve lists and list items."""

    def setUp(self):
        """Create user for the tests."""
        self.credentials = {"username": "test", "password": "12345"}
        self.user = get_user_model().objects.create_user(**self.credentials)
        with Path(mock_path / "import_shelve_with_lists.csv").open("rb") as file:
            self.import_results = shelve.importer(file, self.user, "new")
...
class ImportShelveStatusNormalization(TestCase):
    """Test status normalization during Shelve import."""

    def setUp(self):
        """Create user for the tests."""
        self.credentials = {"username": "test", "password": "12345"}
        self.user = get_user_model().objects.create_user(**self.credentials)
        with Path(mock_path / "import_shelve_status_normalization.csv").open("rb") as file:
            self.import_results = shelve.importer(file, self.user, "new")
