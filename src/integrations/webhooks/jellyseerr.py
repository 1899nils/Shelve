...
class JellyseerrWebhookProcessor:
    ...
    def process_payload(self, payload, user):
        ...
        logger.info(
            "Jellyseerr accepted: user=%s requester=%r type=%s tmdb=%s status=%s -> shelve_status=%s",
            user.username,
            requester,
            shelve_media_type,
            tmdb_id,
        ...
        item = self._get_or_create_tmdb_item(shelve_media_type, tmdb_id)
        if not item:
            return

        self._get_or_create_user_media(user, item, shelve_media_type, desired_status)
