...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if plex_count:
            ...
            self.stdout.write(f"Shelve Unique Tracks:  {validation['unique_tracks']:,}")
            difference = validation["unique_tracks"] - plex_count
            if difference > 0:
                self.stdout.write(
                    self.style.WARNING(
                        f"Difference:                +{difference:,} (Shelve has more tracks)",
                    ),
                )
            elif difference < 0:
                self.stdout.write(
                    self.style.WARNING(
                        f"Difference:                {difference:,} (Shelve has fewer tracks)",
                    ),
                )
            else:
