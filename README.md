...
This fork takes Shelve's solid foundation and enhances it for **daily usability**. While maintaining the core feel of the original, it adds:
...
Important:
- Shelve uses PostgreSQL only when `DB_HOST` is set.
- If `DB_HOST` is not set, Shelve uses SQLite at `/shelve/db/db.sqlite3`.
...
Important:
If you use SQLite, you must persist `/shelve/db`.
...
Do not replace these with `DATABASE_URL`; Shelve will fall back to SQLite if `DB_HOST` is missing.
...
```bash
docker network create shelve-net

docker run -d \
  --name shelve-redis \
  --network shelve-net \
  --restart unless-stopped \
  -v shelve-redis-data:/data \
  redis:8-alpine
...
- For SQLite, mount persistent storage to `/shelve/db`.
- For PostgreSQL, set `DB_HOST`, `DB_NAME`, `DB_USER`, `DB_PASSWORD`, and `DB_PORT` on the Shelve container.
...
- `URLS` - Your public URL if using a reverse proxy (e.g., `https://shelve.mydomain.com`)
...
- SQLite stores the app database at `/shelve/db/db.sqlite3`; persist `/shelve/db`.
...
- Do not assume `DATABASE_URL` enables PostgreSQL. Shelve uses Postgres only when `DB_HOST` is set.
...
2. If you intended to use SQLite, confirm `/shelve/db` is mounted to persistent storage.
...
```yaml
environment:
  - URLS=https://shelve.mydomain.com
```

Multiple origins can be specified with commas: `https://shelve.mydomain.com,https://shelve-alt.mydomain.com`
...
### Original Shelve Features
All the core features from the original Shelve are still here:

| Homepage | Calendar |
|----------|----------|
| <img src="https://cdn.fuzzygrim.com/file/fuzzygrim/shelve/homepage.png?v2" alt="Homepage" /> | <img src="https://cdn.fuzzygrim.com/file/fuzzygrim/shelve/calendar.png" alt="Calendar" /> |

| Media List Grid | Media List Table |
|-----------------|------------------|
| <img src="https://cdn.fuzzygrim.com/file/fuzzygrim/shelve/medialist_grid.png" alt="List Grid" /> | <img src="https://cdn.fuzzygrim.com/file/fuzzygrim/shelve/medialist_table.png" alt="List Table" /> |

| Media Details | Tracking |
|---------------|----------|
| <img src="https://cdn.fuzzygrim.com/file/fuzzygrim/shelve/media_details.png" alt="Media Details" /> | <img src="https://cdn.fuzzygrim.com/file/fuzzygrim/shelve/tracking.png" alt="Tracking" /> |
...
- `:dev` - Exact copy of upstream repo (FuzzyGrim/Shelve)
...
## 📚 Core Features (from Original Shelve)

All the original Shelve features are preserved:
...
Pull requests are welcome! Whether it's fixing bugs, improving documentation, or adding features, your contributions help make Shelve better.
...
This fork is based on [FuzzyGrim/Shelve](https://github.com/FuzzyGrim/Shelve), an excellent self-hosted media tracker. This fork adds enhancements focused on daily usability, performance, and additional media types while maintaining compatibility with the core application.
